...
class oki66207_processor_t(idaapi.processor_t):
    ...
    def _get_current_ea_dd_flag(self, ea):
        # This is kinda fucked up, I'm trying stuff but I can't say it will
        # work for sure

        # We already know the DD flag value for this address
        if ea in self.DD_FLAG_MAPPING:
            return self.DD_FLAG_MAPPING[ea]
        else:
            # We don't know the DD flag for this address. Check if there is any
            # Xrefs to it and if there is, what was the DD flag of the source
            for addr in XrefsTo(ea):
                if addr in self.DD_FLAG_MAPPING:
                    self.DD_FLAG_MAPPING[ea] = self.DD_FLAG_MAPPING[addr]
                    return self.DD_FLAG_MAPPING[addr]

            # No Xrefs, iterate over each previous instruction and check the
            # closest previous DD flag available
    ...
    def _set_current_ea_dd_flag(self, ea, value, force_change_if_already_exist, info_string):
        if ea == 0:
            return

        while self.dd_flag_set_lock == False:
            pass

        self.dd_flag_set_lock = False

        if ea in self.DD_FLAG_MAPPING:
            if value != self.DD_FLAG_MAPPING[ea]:
    ...
    def _get_instruction_from_op(self, ea, op):
        ...
        for index, definition in enumerate(oki66207.INSN_DEFS):
            if definition[oki66207.IDEF_DD] != oki66207.DD_FLAG_UNUSED:
                ...
                # flag has the correct value, else there is no reason to actually even try

                current_dd = idaapi.get_sreg(ea, idaapi.str2reg("ireg_dd"))

                if current_dd == BADSEL:
                    idaapi.split_sreg_range(ea, idaapi.str2reg("ireg_dd"), 0, SR_user)

                #if definition[oki66207.IDEF_DD] == oki66207.DD_FLAG_ONE and self._get_current_ea_dd_flag(ea) != 1:
                if definition[oki66207.IDEF_DD] == oki66207.DD_FLAG_ONE and current_dd != 1:
                    continue

                #if definition[oki66207.IDEF_DD] == oki66207.DD_FLAG_ZERO and self._get_current_ea_dd_flag(ea) != 0:
                if definition[oki66207.IDEF_DD] == oki66207.DD_FLAG_ZERO and current_dd != 0:
                    continue
            ...
            if definition[oki66207.IDEF_OPCODES][0] == ord(op):
                ...
                if match:
                    if definition[oki66207.IDEF_DD] == oki66207.DD_FLAG_RESET:
                        ...
                        #MakeComm(ea, "DD set")
                        idaapi.split_sreg_range(ea, idaapi.str2reg("ireg_dd"), 1, SR_user)
                    else:
    ...
    def notify_ana(self, insn):
        ...
        ea = insn.ea
        idaapi.set_default_sreg_value(idaapi.getseg(ea), idaapi.str2reg("ireg_dd"), 0)

        opcode = get_bytes(ea, 1)[0]
        current, index = self._get_instruction_from_op(ea, opcode)

        # If no instruction was found, try to change the DD flag
        # TODO: verify if that works
        # UPDATE: Seems to be working ok-ish
        if current == None:
            #original_dd = self._get_current_ea_dd_flag(ea)
            original_dd = idaapi.get_sreg(ea, idaapi.str2reg("ireg_dd"))
            new_dd = 1 if original_dd == 0 else 0
            #self._set_current_ea_dd_flag(ea, new_dd, True, "INSTRUCTION NOT FOUND 1 ({0})".format(hex(ea)))
            print(idaapi.split_sreg_range(ea, idaapi.str2reg("ireg_dd"), new_dd, SR_user))
            current, index = self._get_instruction_from_op(ea, opcode)
            ...
            if current == None:
                #self._set_current_ea_dd_flag(ea, original_dd, True, "INSTRUCTION NOT FOUND 2 ({0})".format(hex(ea)))
                idaapi.split_sreg_range(ea, idaapi.str2reg("ireg_dd"), original_dd, SR_user)
                print("{2} Warning - notify_ana: Instruction not found: {0} ({1})".format(ord(opcode), hex(ord(opcode)), hex(ea).replace("L", "")))
                insn.itype = 0x5
                insn.size = len(oki66207.INSN_DEFS[insn.itype][oki66207.IDEF_OPCODES])
                return insn.size # Instruction not found

            else:
                print("{0} Warning - notify_ana: DD flag changed to avoid ILLEGAL instruction".format(hex(ea).replace("L", "")))


        # Index of the correct instruction in the big instructions array
        insn.itype = index

        if self._fill_operands(ea, current, insn):
    ...
    def notify_out_header(self, ctx):
        ctx.out_line(self.assembler["header"])
        ctx.flush_outbuf(0)
    ...
    def _check_dd_when_psw_set(self, insn):
        ...
        if FUNCTIONS_ENTRY_PRINT:
            print("DEBUG: _check_dd_when_psw_set {0}".format(hex(ea)))
        ...
        if "imm8" in mnemonic:
            ...
            if "andb" in mnemonic:
                #new_bit = bit & self._get_current_ea_dd_flag(ea)
                #self._set_current_ea_dd_flag(ea, new_bit, True, "psw & imm8 at {0}".format(hex(ea)))
                new_bit = bit & idaapi.get_sreg(ea, idaapi.str2reg("ireg_dd"))
                idaapi.split_sreg_range(ea, idaapi.str2reg("ireg_dd"), new_bit, SR_user)
        ...
        #    self._set_current_ea_dd_flag(ea, bit, True, "psw+imm16 at {0}".format(hex(ea)))
        else:
    ...
    def notify_emu(self, insn):
        ...
        if FUNCTIONS_ENTRY_PRINT:
            print("DEBUG: notify_emu {0}".format(hex(ea)))
	...
	flow = (features & CF_STOP) == 0
	    ...
	    add_cref(ea, dest, fl_JN)
            #self._set_current_ea_dd_flag(dest, self._get_current_ea_dd_flag(ea), False, "fl_JN at {0}".format(hex(ea)))
            idaapi.split_sreg_range(dest, idaapi.str2reg("ireg_dd"), idaapi.get_sreg(ea, idaapi.str2reg("ireg_dd")), SR_user)
            #print("Creating jump cref at {0} towards {1}".format(hex(ea), hex(dest)))

        elif (features & CF_CALL):
	    dest = insn.Op2.addr
	    add_cref(ea, dest, fl_CN)
            #self._set_current_ea_dd_flag(dest, self._get_current_ea_dd_flag(ea), False, "fl_CN at {0}".format(hex(ea)))
            idaapi.split_sreg_range(dest, idaapi.str2reg("ireg_dd"), idaapi.get_sreg(ea, idaapi.str2reg("ireg_dd")), SR_user)
            #print("Creating call cref at {0} towards {1}".format(hex(ea), hex(dest)))

	if flow:
	    add_cref(ea, ea + insn.size, fl_F)
            #print("Creating flow cref at {0} towards {1}".format(hex(ea), hex(ea + insn.size)))
    ...
    def init_registers(self):
        ...
        for i in xrange(len(self.reg_names)):
            print("Setting register {0}: {1}".format(i, 'ireg_' + self.reg_names[i]))
            setattr(self, 'ireg_' + self.reg_names[i], i)


        '''
        TODO: Find a fucking way to get DD exposed to the user

        Help:
          https://blog.quarkslab.com/ida-processor-module.html#id5
          https://www.hex-rays.com/products/ida/support/idapython_docs/ida_segregs-module.html#copy_sreg_ranges
          https://www.hex-rays.com/products/ida/support/idapython_docs/
        '''
        # Segment register information (use virtual CS and DS registers if your
        # processor doesn't have segment registers):
        # We have at least two here (DD and CF), so we put the index of the
        # first and last SREGS here (from the self.reg_names above)
        self.reg_first_sreg = self.ireg_dd
        self.reg_last_sreg = self.ireg_DS

        # You should define 2 virtual segment registers for CS and DS (IDA requirement).
        # index of CS register
        self.reg_code_sreg = self.ireg_CS
        # index of DS register
